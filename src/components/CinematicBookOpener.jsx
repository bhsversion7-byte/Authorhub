...
function useBookTexture(url, { repeat = [1, 1], colorSpace = THREE.SRGBColorSpace } = {}) {
  const { gl } = useThree();
  ...
  useEffect(() => {
    ...
    loader.load(
      ...
      (nextTexture) => {
        ...
        nextTexture.wrapS = repeat[0] === 1 ? THREE.ClampToEdgeWrapping : THREE.RepeatWrapping;
        nextTexture.wrapT = repeat[1] === 1 ? THREE.ClampToEdgeWrapping : THREE.RepeatWrapping;
  ...
  }, [url, repeat[0], repeat[1], colorSpace, gl]);
...
function useSpineTexture() {
  const [texture] = useState(() => {
    ...
    for (let i = 0; i < 2600; i += 1) {
      ...
      const length = 3 + Math.random() * 16;
      ctx.strokeStyle = Math.random() > 0.5 ? "#8b8174" : "#c7bba7";
      ctx.lineWidth = Math.random() > 0.92 ? 1.2 : 0.45;
      ctx.beginPath();
      ctx.moveTo(x, y);
      ctx.lineTo(x + length * Math.cos(Math.random() * Math.PI), y + length * Math.sin(Math.random() * Math.PI));
      ctx.stroke();
    }
    ...
    ctx.font = "700 76px Arial Black, Impact, sans-serif";
  ...
  return (
    <group ref={pivotRef} position={[0, 0, 0.075]}>
      <mesh castShadow receiveShadow position={[PAGE_WIDTH / 2, 0, 0]}>
        ...
        <meshStandardMaterial attach="material-0" color="#e9e4da" roughness={0.94} bumpMap={insideTexture} bumpScale={0.018} />
        <meshStandardMaterial attach="material-1" color="#f0ece3" roughness={0.96} bumpMap={insideTexture} bumpScale={0.018} />
        <meshStandardMaterial attach="material-2" color="#eee8dd" roughness={0.96} bumpMap={insideTexture} bumpScale={0.018} />
        <meshStandardMaterial attach="material-3" color="#d8d0c4" roughness={0.96} bumpMap={insideTexture} bumpScale={0.018} />
        <meshStandardMaterial
          ...
          toneMapped
        />
        <meshStandardMaterial attach="material-5" map={insideTexture} bumpMap={insideTexture} bumpScale={0.026} color="#f7f0e6" roughness={0.98} />
      </mesh>
    </group>
  );
}
...
function BackCover({ coverGeometry, insideTexture }) {
  return (
    <mesh castShadow receiveShadow position={[PAGE_WIDTH / 2, 0, -0.115]}>
      <primitive attach="geometry" object={coverGeometry} />
      <meshStandardMaterial attach="material-0" color="#d9d0c4" roughness={0.95} bumpMap={insideTexture} bumpScale={0.02} />
      <meshStandardMaterial attach="material-1" color="#e8e1d6" roughness={0.95} bumpMap={insideTexture} bumpScale={0.02} />
      <meshStandardMaterial attach="material-2" color="#ece5d9" roughness={0.95} bumpMap={insideTexture} bumpScale={0.02} />
      <meshStandardMaterial attach="material-3" color="#d8cfc2" roughness={0.95} bumpMap={insideTexture} bumpScale={0.02} />
      <meshStandardMaterial attach="material-4" color="#efe8dc" map={insideTexture} bumpMap={insideTexture} bumpScale={0.026} roughness={0.98} />
      <meshStandardMaterial attach="material-5" color="#f3eee5" map={insideTexture} bumpMap={insideTexture} bumpScale={0.026} roughness={0.98} />
    </mesh>
  );
}

function Spine({ motion, spineTexture, insideTexture }) {
  ...
  return (
    <mesh ref={spineRef} castShadow receiveShadow position={[-0.12, 0, -0.015]}>
      ...
      <meshStandardMaterial attach="material-0" map={spineTexture} bumpMap={insideTexture} bumpScale={0.035} roughness={0.96} />
      <meshStandardMaterial attach="material-1" map={spineTexture} bumpMap={insideTexture} bumpScale={0.035} roughness={0.96} />
      <meshStandardMaterial attach="material-2" color="#f0ebe1" bumpMap={insideTexture} bumpScale={0.03} roughness={0.96} />
      <meshStandardMaterial attach="material-3" color="#d9d0c4" bumpMap={insideTexture} bumpScale={0.03} roughness={0.96} />
      <meshStandardMaterial attach="material-4" color="#0c0a09" roughness={0.9} />
      <meshStandardMaterial attach="material-5" map={spineTexture} bumpMap={insideTexture} bumpScale={0.035} roughness={0.96} />
    </mesh>
  );
}
...
function TurningSheet({ index, total, motion, pageGeometry, insideTexture }) {
  ...
  const pageTint = index % 3 === 0 ? "#f4efe5" : index % 3 === 1 ? "#f8f2e8" : "#eee5d8";
...
function PageBlockEdges({ motion, insideTexture }) {
  ...
  return (
    <mesh ref={edgeRef} castShadow receiveShadow position={[PAGE_WIDTH / 2 - 0.03, 0, -0.052]}>
      ...
      <meshStandardMaterial map={insideTexture} bumpMap={insideTexture} bumpScale={0.018} color="#e7dfd3" roughness={0.98} />
    </mesh>
  );
}
...
function BookModel({ motion, scrollProgress, autoOpen }) {
  ...
  const coverTexture = useBookTexture(BOOK_ASSETS.cover);
  const insideTexture = useBookTexture(BOOK_ASSETS.inside, { repeat: [1.05, 1.05] });
  const spineTexture = useSpineTexture();
  ...
  return (
    <group ref={groupRef} position={[2, 0, 0]} rotation={[-0.34, -0.58, 0.12]}>
      <Spine motion={motion} spineTexture={spineTexture} insideTexture={insideTexture} />
...
function BookLights() {
  return (
    <>
      ...
      />
      <spotLight
        position={[2.8, 3.0, 3.8]}
        angle={0.46}
        penumbra={0.86}
        intensity={0.9}
        color={AUTHORHUB_COLORS.sunsetOrange}
        distance={9}
        decay={2}
      />
