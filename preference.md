...
- Collaboration RPCs are intentionally `SECURITY DEFINER` only where they break necessary RLS recursion or perform an atomic owner action. Every such function must set a fixed search path, verify `auth.uid()` internally, and revoke anonymous execution unless a public token-read endpoint explicitly requires it.
- Keep production schema history append-only and reconcile any deployment-history discrepancy with a new, reviewed migration; never rename or rewrite an applied migration file.
