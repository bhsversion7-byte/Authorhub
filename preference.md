...
- Relation graph node halo (`.planet-halo`): non-main characters use fixed `#8BA09C` at low opacity (same reasoning as above). Main-tagged characters get a two-tone halo the user considers a locked, loved design: light-yellow outer ring (`stroke: rgba(230,178,102,0.85)`) and light-red inner glow (`fill: rgba(224,122,104,0.42)`). These two hues were deliberately tuned close together on the color wheel (amber-gold vs. coral, not pale-yellow vs. pink-red) so the ring reads as one smooth warm gradient instead of two clashing colors - the user flagged the first attempt as "内外环颜色不够接近，看起来突兀" (inner/outer too far apart, looks jarring) and this is the corrected version. Size/opacity numbers (`radius+21`, `stroke-width 8`, `opacity 0.52`) are unchanged - only change the hues (keeping them close together), never the size or shape, if this needs revisiting.
...
- These two cards' `panel-title` header (icon + "大纲"/"设定集" heading) already showed the section name once; an earlier pass also let `FocusTextarea` render its own internal label text row underneath, duplicating "大纲"/"设定集" on two lines. Fixed by giving `FocusTextarea` a `hideLabel` prop (suppresses its internal label+button row entirely) and a `forwardRef`-exposed `open()` method, so `NovelSection` renders a single `.panel-title-expand` trigger button directly inside the existing `panel-title` flex row instead. Do not reintroduce a second label row for these two cards - the expand trigger must live in the same row as the heading, not stacked below it (a plain CSS `position:absolute` attempt to fake this alignment failed because a global rule - `.novel-section :where(.novel-hero, .story-card, ...) > * { position: relative !important; }` - forces every direct child of `.story-card` into its own positioning context, so an absolutely-positioned descendant resolves against the wrong ancestor; the ref-based in-row button sidesteps that entirely).
...
- Image uploads (`MediaCarousel`) are capped at 2MB per file (`MAX_IMAGE_BYTES`; was 5MB, lowered 2026-07-03). The size hint lives inside the image frame's own empty-state text ("添加 2 张或更多图片后，可滑动查看。单张图片最大 2MB。"), not as a separate line outside the frame. Both sentences are the same `.media-empty-hint` span/class and must render at the same (small, 12px) font-size - `.media-empty`'s own font-size is deliberately tied to the user's global reading-font-size setting (`var(--editor-font-size)` `!important` in `material-system.css`) for normal reading text elsewhere, so these two helper lines need their own explicit same-size class rather than relying on inheriting the container's font-size, or they visibly mismatch. A rejected oversized file shows a filled, clearly visible pill-style message (`.media-error`), not a barely-there grey line - so it doesn't read as "the upload button is broken" when the real cause is a file being over the limit. The "添加图片" button/upload pipeline itself was re-verified working end-to-end (real Storage upload round trip) when this was investigated.
...
- Refreshing the page must keep the user on whatever they were looking at (author dashboard, user center, a specific private/shared novel) - it must never bounce back to the author homepage just because the app remounted. `App.jsx` persists `activeView` to `localStorage` (`author-hub-active-view`), restores it as the initial state, and clears it on logout/unregister. This is pure UI navigation state, never part of the `data` object saved to Supabase - it must stay that way. If the restored view points at a novel that no longer exists (deleted elsewhere, share revoked while away), it falls back to the author dashboard automatically once both private and shared novels have finished loading (a `sharedNovelsReady` flag avoids a one-frame flash of the "这里还没有小说" empty-state while shared novels are still being fetched after a restored `shared-*` view).
...
- 2026-07-03 (later night): tuned the main-character halo's two hues closer together (amber-gold outer / coral inner, see updated values above) after the user flagged the first version as too jarring between yellow and red. Made the two image-upload hint lines ("添加 2 张或更多图片后..." / "单张图片最大 2MB。") render at the same explicit 12px instead of one inheriting the dynamic reading-font-size and the other being a fixed smaller value. Fixed refresh always bouncing to the author homepage: `activeView` now persists to `localStorage` and is restored on load (pure UI state, confirmed never touches the Supabase-synced `data` object - documented above and re-verified by a background audit agent). Removed a duplicate "大纲"/"设定集" label - `FocusTextarea` previously always rendered its own internal label row even when the surrounding card already showed the same heading in `panel-title`; added a `hideLabel` prop plus a `forwardRef`-exposed `open()` method so the expand trigger now renders once, inside the same row as the heading, instead of twice on two lines. Re-ran a full sync audit (read-only agent) covering the three changes above plus every character/event/novel/clear-all delete path, the cloud-save debounce/flush/error-swallow behavior, and MediaCarousel's upload/remove handlers - came back clean, no data-loss gaps found.
