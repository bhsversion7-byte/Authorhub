...
- **Fix (additive, no visual/behavior change):** `main.jsx` listens for `vite:preloadError` and reloads once (sessionStorage-guarded against loops) to fetch the fresh build; `ErrorBoundary` also reloads if a chunk-load error reaches it; `vercel.json` now serves `/assets/*` with `immutable` caching. The blank can no longer persist across a deploy.
