...
- Image uploads (`MediaCarousel`) are capped at 5MB per file (`MAX_IMAGE_BYTES`); oversized files are silently skipped with a toast, and the 5MB limit is shown as static hint text under the upload controls (not just discovered after a rejected upload).
